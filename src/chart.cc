...
void Chart::draw(NFont &font, NFont &fontSmall, SDL_Renderer *render, unsigned h) {
  ...
  if (!processList.empty()) {
    ...
    for (auto it = processList.begin(); it != processList.end(); ++it) {
      ...
      drawTextWithOutline(render, pString, fontSmall, bar.x + bar.w + xoff, yoff + SMALL_PT*ix, 1, NFont::Color(color.r, color.g, color.b));
      ...
      SDL_RenderDrawLine(render, bar.x + bar.w + xoff-5, yoff + SMALL_PT*ix + font.getHeight("H")/2, bar.x + bar.w + 1, (r.y + r.h) - r.h/2);
    }
    ...
    for (unsigned i = 0; i < processList.size(); ++i) {
      ...
      drawTextWithOutline(render, os.str(), fontSmall, bar.x + bar.w + xoff + width, yoff + SMALL_PT*i, 1, NFont::Color(color.r, color.g, color.b));
      ...
      drawTextWithOutline(render, " MB", fontSmall, bar.x + bar.w + width + xoff + width0, yoff + SMALL_PT*i, 1, NFont::Color(color.r, color.g, color.b));
    }
  }
}
